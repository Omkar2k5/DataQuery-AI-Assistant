...
import { analyzeData, DataSchema, QueryResult } from './utils/api';
...
function App() {
  ...
  const handleQuerySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add the assistant's response to the conversation with natural language answer
      setConversation([
        ...newConversation,
        { 
          role: 'assistant' as const, 
          content: result.answer // Show the natural language answer in conversation
        }
      ]);
      ...
      setConversation([
        ...
        { 
          role: 'assistant' as const, 
          content: `I couldn't understand that. ${error instanceof Error ? error.message : 'Something went wrong'}` 
        }
      ]);
  
      // Set error result
      const errorResult: QueryResult = {
        answer: `Error: ${error instanceof Error ? error.message : 'An unexpected error occurred'}`,
        sqlQuery: '',
        needsChart: false,
        chartType: null,
        chartDataColumn: ''
  ...
  const generatePDF = async () => {
    if (!resultsRef.current || !queryResult) return;
    ...
    // Add title
    pdf.setFontSize(20);
    pdf.setTextColor(66, 102, 241); // Indigo color
    pdf.text('DataQuery AI Assistant', 20, 20);
